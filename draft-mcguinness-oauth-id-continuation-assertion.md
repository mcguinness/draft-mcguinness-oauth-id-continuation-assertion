...
3. The handle crosses a trust boundary only inside an ID-JAG (to the RAS) or
   ...
   intra-domain context ({{transaction-token-context}}).
...
6. a recheck against authoritative RAS state confirms the authorization
...
The IdP MUST apply its local tenant and issuer policy to a nominated issuer
before using it; a RAS nomination alone MUST NOT trigger metadata retrieval or
other dereferencing. When the CAI issuer identifier is also an OAuth
authorization server issuer identifier, the IdP SHOULD obtain the CAI's
signing keys from the `jwks_uri` in that issuer's authorization server
metadata, located and retrieved over TLS per {{RFC8414}}, Section 3, and
refresh them on rotation. Metadata that is missing, is malformed, omits
`jwks_uri`, or whose `issuer` does not exactly equal the authorized identifier
({{RFC8414}}, Section 3.3) MUST NOT supply verification keys. The IdP uses only
a `jwks_uri` from such validated metadata, never a `jwks_uri` or key set the
Resource Authorization Server supplies. For any other CAI, or when discovery
yields no valid keys, the IdP uses authenticated configuration or rejects the
assertion.
...
An IdP that dereferences a nominated issuer's metadata ({{metadata}}) applies
the usual server-side request-forgery safeguards: it validates TLS, bounds
redirects and response size, and refuses internal or link-local addresses.
...
({{transaction-token-context}}). Neither the Scheduler nor BriefingAgent
selects H0.
