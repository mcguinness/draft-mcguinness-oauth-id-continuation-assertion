...
The IdP returns a grant that one downstream Resource Authorization Server
(RAS) accepts. Each RAS knows the user by a pairwise subject that only the IdP
can resolve.
...
further service for the same user. It does so after the user has gone, or at
an audience the credential never addressed. It holds no credential of the
user's to exchange. It cannot name the user for the next audience, and the
access token it received gives the next RAS nothing it trusts.

This document defines the Identity Continuation Assertion: a short-lived,
sender-constrained JSON Web Token (JWT) {{RFC7519}} that such a workload
obtains from a Continuation Assertion Issuer (CAI). The CAI is a party the IdP
trusts to report what the RAS that issued the workload's access token has
accepted. The assertion states two facts. First, the RAS accepted the
referenced ID-JAG, established an authorization from it, and that
authorization remains active and permits continuation. Second, the
authenticated workload is the party now acting on that authorization and
identifies the key it will prove to the IdP. The workload presents the
assertion to the IdP as the subject token of an OAuth 2.0 Token Exchange
...
to it, and issues the next ID-JAG.
...
accepted authorization being continued and its current actor. The continuation
request selects the target and the requested authority. The scopes of the
...
the first ID-JAG runs unmodified ID-JAG, which this document calls the base
profile, and so does any RAS from which no workload continues. Only the
continuing workload, its RAS and CAI, and the IdP need support this document.
The IdP keeps the chain records and enforces the tenant's policy and replay
protection. That work sits at the IdP because it already resolves the pairwise
subject and holds that policy. An ID-JAG deployment can therefore add
multi-hop access where a request's path is not known in advance, as at a tool
gateway for agents, including Model Context Protocol (MCP) gateways
({{example-gateway}}).

This document complements OAuth Identity Chaining
{{I-D.ietf-oauth-identity-chaining}} ({{rationale-idjag}}). It is not a
substitute for narrowing an existing token within one trust domain
({{decision-rule}}). Its scope is the two exchanges above: the workload's
request to the CAI for an assertion, and its exchange of that assertion at the
IdP. That scope covers the assertion and handle they carry and the obligations
of the IdP, the continuing workload, its RAS, and the CAI. How a handle
travels inside a trust domain is left to the deployment
...
1. As in ID-JAG, AgentApp exchanges Alice's ID Token at the IdP for an ID-JAG
   ...
   as it stands at each continuation. The IdP also puts a handle for H0 in the
...
4. **New:** the gateway exchanges that access token at its RAS's token endpoint
   ...
   hop bound to the token. If the authorization is still active, the RAS
   issues the assertion attesting that H0 is active and that this gateway
   holds it ({{assertion-issuance}}).
...
: One link of a chain: the IdP's record of an ID-JAG it issued, with an
  ...
  ACCEPTED when a RAS redeems its ID-JAG
  and binds the handle ({{ras-processing}}, {{hop-activation}}); a hop from
  which no workload continues is terminal, while sibling branches may
  continue.
...
: The IdP's recorded chain context and continuation policy for a chain: the
  facts fixed at the root exchange, and the continuers, targets, and limits
  the IdP reads under tenant policy as it stands ({{root-envelope}}). The IdP
  evaluates every continuation against it ({{validation}}).

Authorization basis:
: A reference to tenant policy, recorded in the envelope in place of an
  enumerated target list, that the IdP reads at each continuation to decide
  which targets a chain may reach ({{root-envelope}}).
...
: REQUIRED. The current actor presenting the Token Exchange request, encoded
  ...
  * `iss` and `sub` are REQUIRED, non-empty strings: the actor's canonical
    ...
    assertion is, they equal the credential's `iss` and `sub`.
  * The IdP compares both with the canonical actor identity of the
    authenticated client ({{client-identity}}).
  * Only `iss` and `sub` form the canonical actor identity under this
    document. Additional members may carry further information about the
    actor but MUST NOT affect processing defined by this document unless
    another specification profiles their use. A recipient MUST ignore
    members it does not understand, and `exp`, `nbf`, `aud`, `scope`, `cnf`,
    and nested `act` MUST NOT be present.
...
Other top-level claims may appear and MUST be ignored for validation,
...
| Continuing workload | Assertion Issuance Request, Client Authentication, and Successful Response ({{assertion-token-exchange}}, {{assertion-client-auth}}, {{assertion-response}}); Continuation Request and Client Authentication ({{request}}, {{client-identity}}); Successful Response and Error Response and Recovery ({{success-response}}, {{error-response}}) |
...
extends it to issue a continuation-capable ID-JAG. This document defines no
processing for an `actor_token` on the root exchange: the base profile leaves a
presented one outside its scope
...
IdP establishes the chain by issuing the root ID-JAG with a fresh root handle
...
| Chain identity | The root actor | root exchange | fixed |
...
Token claims cannot supply these values. No later policy or request changes
the parts the table marks fixed. Within the envelope, the IdP authorizes each
grant subject to the governing authorization and current tenant policy, as in
ID-JAG. How a policy change reaches a running chain therefore depends on the
form the IdP recorded:

1. An envelope that enumerates its targets MUST NOT admit a target the
   tenant adds after establishment ({{example-dynamic}}); a target the tenant
   removes is refused from the next continuation.
2. A basis-referenced envelope admits whatever the basis currently reads,
   such as a service's classification or a group's membership
   ({{example-gateway-root}}). A basis names a class of targets: a policy
   change may alter the class's membership but not what the class means, and
   a tenant that wants a different authorization defines a new basis rather
   than redefining an existing one ({{security-envelope}}).
3. The permitted continuers and the limits are tenant controls, read as
   policy stands in either direction, so a withdrawn continuer or a
   tightened limit takes effect at the next continuation.
4. The root request's audience and scope bound only the root ID-JAG, not
   what the chain may later reach.
5. Target authority widens only through a basis.
6. Acceptance at a RAS does not change the envelope; see
   {{hop-activation}}.

Every continuation is evaluated against the envelope (the
envelope-containment rule of {{validation}}).
...
* All successful redemptions of one continuation-capable ID-JAG, identified
  by its validated issuer and handle within the authenticated tenant binding,
  MUST resolve to the same hop binding, so that no redemption creates a
  distinct continuation source. A matching handle under another issuer is a
  different grant, not a retry.
...
and neither it nor any ancestor is revoked. The validation rules of the
continuation exchange decide whether a particular continuation from it
succeeds ({{validation}}).
...
deployment designates. For a RAS acting as its own CAI, that is the RAS's
...
of the client's own key. The CAI MUST verify that proof. It binds the
...
In this exchange the `subject_token` identifies the RAS authorization context
from which continuation is requested; its subject is not copied into the
resulting assertion.
...
The CAI SHOULD use a live recheck where the tenant requires withdrawal of a
hop's authorization to stop fresh assertions before the RAS's token would
expire.
...
and the actor's client authentication ({{client-identity}}):
...
`act` is the authenticated client ({{client-identity}}), and a request carrying
either parameter is rejected with `invalid_request` ({{error-response}}).
...
the client supplies no other canonical actor identity, the identity authority
is the IdP's own issuer identifier and `sub` is the `client_id`. That is the
case for a client authenticated with a client secret or a mutual-TLS
certificate and registered without a workload identity.
...
Where the client authenticates with an {{RFC7523}} client assertion whose `sub`
is its `client_id`, the canonical actor identity is that assertion's issuer and
that `client_id`, and the IdP MUST authorize that issuer for that client
({{rationale-client-id}}).
...
3. **Issuer trust.**
   ...
   * in either case, that issuer is trusted for the chain's tenant, recorded at
     ...
     identity authority ({{client-identity}});
...
5. **Current actor and binding.**
   ...
   * the request proves possession of the `cnf` key with a matching DPoP
     ...
     accepted as a bearer token ({{RFC7800}});
   * the actor is one the chain's envelope permits to continue from the
     presented hop ({{root-envelope}}); and
...
7. **Envelope containment.** The effective authorization the IdP would grant,
   ...
   envelope, that is:
   * consistent with the facts fixed at the root exchange;
   * within what the envelope permits, as recorded at establishment and
   ...
   * within current IdP actor policy.
   ...
   `authorization_details` values that express that authorization. The IdP
   ...
   the comparison rules of each detail type. The IdP rejects a detail type
...
`identity_continuation_handle` claim ({{chain-id}}), a claim inside
`access_token` and not a separate Token Exchange response parameter. The
accepting RAS binds it ({{ras-processing}}), and the CAI reaches it through
RAS state or intra-domain context ({{handle-propagation}}).
...
grant-anchored chain re-roots without the user ({{lifecycle-grant}}); and a
handle disabled by withdrawn permission to continue cannot re-root at all.
...
CAI. A fresh assertion is not guaranteed: the subject token may have expired,
or the lost success may have consumed the remaining depth, fan-out, or hop
budget ({{lifecycle-limits}}). It may create an equivalent grant and sibling
hop but no additional authority. On `invalid_request` in general the client
corrects or abandons the request, since a malformed request or a prohibited
parameter fails again with a fresh assertion, and blind reacquisition loops
and loads the CAI.
...
policy event or at the next continuation attempt. An ended chain stays ended:
restoring the policy that withdrew permission does not revive it, its handles
remain permanently unusable ({{error-response}}), and a new chain is needed.
...
| IdP removes a CAI or its keys ({{issuer-trust}}) | Unaffected: the CAI may keep issuing | Fails issuer trust at the next evaluation, for existing chains and for assertions already issued | Not needed; configure a replacement CAI |
...
and the run still derives the handle from durable RAS task authorization
rather than from a scheduler-held handle ({{handle-propagation}},
...
A Resource Authorization Server advertises separately that it recognizes a
continuation-capable ID-JAG and binds the `identity_continuation_handle`
claim to authorization state ({{ras-processing}}). It does so by listing the
grant profile `urn:ietf:params:oauth:grant-profile:id-jag-continuation` in its
`authorization_grant_profiles_supported`
{{I-D.ietf-oauth-identity-assertion-authz-grant}}. This value is distinct
...
Provisioning is out of band and deployment-specific. A tenant registers a CAI
at the IdP as the record of that trust scope: the issuer identifier, its keys
or `jwks_uri`, the RAS whose hops it attests, the tenant, and the actor
identity authority it pairs with. A tenant withdraws the CAI the same way.
...
It then reuses the access token it redeems there while that token remains
valid and covers the requested access. Calls for another user, tenant, chain,
or key need their own continuation. Reusing a token across chains would attach
later calls to the earlier chain's handle and lineage.

The first call in a context therefore costs the assertion exchange, the
continuation exchange, and the redemption. Later calls in the same context
cost nothing more while the token lasts.

Renewal is another continuation with a fresh assertion, and it succeeds only
...
does not relax the replay rules of {{validation-replay}}. Whether the hop
tree could be replaced by self-verifying handles is an open question
...
the same CAI requirements. Either CAI can issue the assertion from its token
...
matching fingerprint waits for or retries that result. An IdP that does not
offer retry rejects the second presentation and needs no more than the
reservation itself.
...
5. **Actor impersonator.** Actor identity is the IdP's per-tenant mapping from
   client authentication and `act` must match it; the actor's identity
   authority is one the IdP configures and pairs with the CAI for that tenant
   ({{client-identity}}, {{security-actor-issuers}}, {{validation}}).
...
17. **Correlator.** Per-hop handles without user data, pairwise subjects,
    `sid` never carried, lineage narrowable ({{chain-id}},
    {{onward-id-jag}}, {{privacy}}).
...
({{validation}}, {{client-identity}}). Because the onward ID-JAG and the
accepting RAS's access token are bound to that same key ({{client-identity}},
{{ras-processing}}), possession is demonstrated continuously from the first
continuation on, not once at issuance. A terminal
...
profile's bearer-token exposure at the ingress, not an exposure this profile
creates, and a RAS that sender-constrains its tokens removes it. This profile
adds no proof requirement at the root ({{root-establishment}}). Continuation
rests on the continuing actor's key, the RAS binding, and the CAI's
attestation, not on the root client's key.
...
The freshness rule bounds the window, and single-use ({{validation-replay}})
confines a consumed assertion to the one grant it first obtained. Without
single-use, an actor whose RAS-local authorization had lapsed, and whom the
CAI would therefore refuse a fresh assertion, could keep continuing from a
consumed one, to any target the envelope permits, until it expired. Idempotent
recovery after a lost response is optional and does not reopen replay
({{idempotent-retry}}).
...
How broad a basis-form envelope is depends on the policy it references
({{root-envelope}}). A gateway that chooses its upstream at request time is
the intended case and also a confused-deputy surface, since a compromised or
misdirected workload can steer continuation to any target the basis admits.
...
reads that classification ({{root-envelope}}), so a basis should name a
...
governs ({{root-envelope}}). A tenant manages that list with the same care as
a basis.
...
CAI for the chain's tenant ({{client-identity}}, {{validation}}). An
accompanying CAI assertion does not relax that: CAI attestation of the hop and
authentication of the actor are independent checks ({{security-trust-model}}).
...
{{security-trust-model}}). Separating the CAI isolates keys
...
additionally attest a hop the RAS refused. In both topologies, the IdP does
not observe RAS-local authorization revocation after issuance, and a separate
CAI adds any delay in RAS state reaching it. {{lifecycle-ending}} gives the
resulting window and the live recheck that closes it. The root envelope still
bounds the result.
...
when a gateway registers everywhere under one name. The profile keeps them
distinct so that lineage records who acted while the grant records who may
redeem.
...
tenant trusts is the natural form of a workload credential and authenticates
the client under {{RFC7523}} ({{client-identity}}). An extension may define
additional actor evidence that tenant policy can require, naming the same actor
as the authenticated client. The shared key across the assertion and the onward
ID-JAG is a consequence of a single proof method, not a property continuation
...
encryption unnecessary. A single compact signed form removes an
...
deployment that wants the work itself to narrow downstream authority expresses
that in the governing authorization, not in RAS scopes.
...
and the CAI's attestation item 2. The canonical actor identity serves item 3,
and sender constraint of the assertion and the onward ID-JAG serves item 4.
The envelope under current policy serves item 5, so that the effective
authority at any continuation is what the envelope permits under current
policy, never more. The anchor serves item 6. Single-use limits one
attestation to one continuation decision, and the lifetime bound limits how
stale that attestation can be ({{validation-replay}}); together they are the
freshness half of item 1. The CAI's verification of the proof is defense in
depth; item 4 is satisfied at the IdP. Where this document offers a choice,
such as the form of acceptance evidence, the choice is between mechanisms that
satisfy the same item.
...
canonical actor identity. Any sender-constrained credential the IdP accepts
under {{client-identity}} would serve; this choice is illustrative. It is
addressed to the IdP alone: the earlier exchange at GatewayRAS
...
The IdP applies the tenant's policy as it stands at each continuation: which
services the tenant classifies as productivity tools, what access it allows
agents to them, and whether `tool-gateway` may continue. A wiki the tenant adds
to that class after this exchange is therefore admitted in
{{example-gateway-continue}} without a new chain, and one it removes is refused
...
operate. It reads H0 from the token and rechecks that the binding is still
active and records continuation as permitted, then issues the assertion bound
to the proven key ({{assertion-preconditions}}).
...
Enterprise policy permits continuation to Expense, Travel, and Booking by
designated workloads, so the IdP records an envelope that enumerates the
targets ({{root-establishment}}); the gateway example shows the other form, an
authorization basis with no enumerated targets.
...
ExpenseRAS binds H0 to the authorization state behind AT1 as GatewayRAS did in
{{example-gateway-bind}}. The carrier is what differs: the handle stays in this
record, and AT1 does not carry it.
...
Expense CAI verifies the Transaction Token for its domain and reads H0 from
...
DPoP proof of the same key, requesting an ID-JAG for TravelRAS. That
sender-constrained credential is its client authentication and resolves to its
...
&subject_token_type=urn:ietf:params:oauth:token-type:identity-continuation
&client_assertion_type=urn:ietf:params:oauth:client-assertion-type:jwt-bearer
&client_assertion=<sender-constrained expense-service credential>
~~~

The IdP validates the exchange ({{validation}}): Expense CAI is trusted for
ExpenseRAS, the RAS recorded for H0, rather than being that RAS itself; H0
names an accepted hop on an active chain; `act` names `expense-service`, the
authenticated client, and the DPoP proof matches `cnf`; and TravelRAS,
TravelAPI, and `trips.read` match the envelope's enumerated Travel entry
rather than a policy basis. The IdP never calls ExpenseRAS; the assertion is
its evidence of acceptance ({{hop-activation}}). It resolves the user's Travel
...
envelope's Booking entry. It authenticates with its own sender-constrained
credential, which resolves to its canonical actor identity as
`expense-service`'s did ({{client-identity}}). The IdP creates H2 under H1 and
extends the lineage:
...
The platform domain is the identity authority for its own workloads, so the
...
1. **Signed assertion versus a recipient-bound direct profile.**
   ...
   worth the added trust configuration
...
8. **Response parameter naming.** This document reuses the Token Exchange
...
proof methods ({{metadata}}); a non-user root
...
* Editorial consolidation: a conceptual Introduction and a shorter Protocol
  Overview; the decision rule moved to Design Rationale; essential terms
...
* Security Considerations open with a fixed record of what the profile
  ...
  changed.
* Removed the RAS metadata parameter `identity_continuation_issuers`, its
  IANA entry, and the Metadata Disclosure considerations: out-of-band trust
  configuration is now the only way an IdP learns a CAI. Issuer Trust
  Configuration keeps the general controls advertisement carried, current-state
  evaluation and key sources, and how a tenant provisions and withdraws one.
* Removed `actor_token` and `actor_token_type`: client authentication is the
  only actor authenticator, so a continuation exchange that carries either
  parameter is rejected as an invalid request, and this document defines no
...
* The Root-Chain Envelope table and a six-rule list are now the only
  statement of envelope semantics; the prose that restated them in that
  section, in Envelope Enforcement and Offline Attenuation, and in the design
...
* Editorial pass for plain language; no normative change. Long sentences are
  split so that each carries one idea, a few passive statements now name the
  acting party first, and the error-code list in Error Response and Recovery
  is a nested list rather than one run-on sentence.
