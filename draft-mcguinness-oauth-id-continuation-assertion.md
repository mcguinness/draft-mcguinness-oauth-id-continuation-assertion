...
subject token. It lets an IdP Authorization Server (IdP) issue an onward
Identity Assertion JWT Authorization Grant (ID-JAG) when a user's request
crosses service boundaries to an audience the requesting workload's credential
never addressed, whether the user is still present or has gone. The profile
targets deployments in which several Resource Authorization Servers trust one
IdP and use pairwise subject identifiers that only the IdP can resolve. It
complements offline attenuation for intra-domain fan-out that does not change
the subject.
...
In many deployments, applications and the APIs they call sign users in through
a common identity provider. The Identity Assertion JWT Authorization Grant
(ID-JAG) {{I-D.ietf-oauth-identity-assertion-authz-grant}} lets an application
exchange the user's identity assertion at that IdP Authorization Server (IdP)
for a grant that one downstream Resource Authorization Server (RAS) accepts.
Each RAS knows the user by a pairwise subject that only the IdP can resolve.
...
* The authenticated workload is the party now acting on that authorization and
  identifies the key it will prove to the IdP.
...
workload may continue to it, and issues the next ID-JAG.
...
One design choice underlies all of this. Continuation travels as a signed
assertion from a party the IdP trusts to speak for the RAS, not as a
recipient-bound credential the IdP issues directly to the workload that will
continue. The IdP never learns that a RAS accepted a grant except through such
an attestation, so the assertion is what admits a workload to the next
exchange ({{rationale-grant-type}}). Whether a recipient-bound credential from
the IdP could serve instead is recorded as an open question ({{open-items}}).
...
the requested authority. The scopes of the access token the workload holds do
...
basis. Only the continuing workload, its RAS and CAI, and the IdP need support
this extension. For that RAS, support means advertising the continuation grant
profile and the grant types it depends on ({{metadata-ras}}), binding each
hop's handle to the authorization state it establishes, and issuing DPoP-bound
access tokens for key-bound ID-JAGs ({{ras-processing}}). The root
client uses the base exchange, but chain establishment requires a resolvable
session or grant anchor ({{lifecycle-anchors}}); without one, the IdP issues
an ordinary ID-JAG without a handle. Every onward target, including a terminal
RAS, needs the base profile's DPoP-bound JWT grant support
...
Its scope is three exchanges: the root exchange, at which the IdP decides
whether to establish a chain and carries the root handle
({{root-establishment}}); the workload's request to the CAI for an assertion;
and its exchange of that assertion at the IdP. With them come the assertion
and handle they carry and the obligations of the IdP, the continuing workload,
its RAS, and the CAI. Which carrier moves a handle inside a trust domain is
left to the deployment; that the CAI takes the handle bound to the
authorization the call's credential identifies is not
({{handle-propagation}}). This document defines no new access-token format,
and a CAI never names the user for the target audience.

## Protocol Overview {#protocol-overview}

This section walks through one continuation; {{example-gateway}} shows the
same steps with every artifact spelled out. Alice uses an agent application,
AgentApp, that calls a tool gateway on her behalf, and the gateway must call a
wiki API for her.

The IdP is the authorization server Alice signed in to; the gateway and the
wiki each sit behind their own RAS. The gateway is the workload, and its RAS
also performs the CAI role, as in the baseline deployment
({{deployment-topologies}}).

Steps are numbered as in the figure; "as in ID-JAG" marks what is unchanged
from the base profile, "new" what this profile adds.
...
1. As in ID-JAG, AgentApp exchanges Alice's ID Token at the IdP for an ID-JAG
   ...
   The IdP carries a handle for H0 in the ID-JAG's
...
2. As in ID-JAG, AgentApp presents the ID-JAG to the gateway's RAS and
   ...
   authorization it creates, so it can later say which hop that token belongs
...
The assertion of step 4 is how the IdP learns that the hop was accepted and is
still active; that is why it accepts an assertion about a hop only from that
hop's RAS or from a CAI it trusts to speak for that RAS. No party other than
the IdP names Alice for the wiki.
...
referenced hop, not an {{RFC7523}} JWT-profile assertion.
...
({{security-topology}}); {{handle-propagation}} limits where it travels.
...
{{protocol-overview}}. Its subsections follow that flow: the root exchange that
establishes a chain, the RAS processing that makes a hop continuable, how the
handle reaches the CAI inside the domain, the CAI's issuance of an assertion,
and the continuation exchange that extends the chain. Each role's requirements
...
normal subject token, such as an ID Token, refresh token, or SAML assertion:
...
This document defines no processing for an `actor_token` on the root exchange:
the base profile leaves a presented one outside its scope
...
root subject token resolves to a lifecycle anchor ({{lifecycle-anchors}}). To
...
a refresh token's OAuth grant. When the subject token resolves no anchor, the
IdP issues the ID-JAG without a handle, as under the base profile. No request
parameter asks for a chain, and advertised support ({{metadata-idp}}) signals
...
2. bind `identity_continuation_handle`, the ID-JAG's issuer, the tenant
   ...
   authorization state it establishes, and record whether continuation is
   permitted under the RAS's own policy; and
...
This document defines no claim that conveys the tenant to the RAS; how a
deployment conveys it to the RAS, and how the RAS derives it, is
deployment-defined.
...
once the RAS has ACCEPTED it: the accepting RAS attests its own hops
first-hand, and any other trusted CAI confirms acceptance and activity by the
RAS's authorization semantics before attesting ({{assertion-issuance}}).

An issued hop that no trusted CAI attests therefore yields no assertion and
reaches no continuation exchange. Absent CAI compromise
...
RAS acceptance gates continuation; the IdP authorizes downstream access
under the chain's governing authorization and current policy ({{validation}}).
RAS-local scopes do not independently authorize that access or define its
limits: a scope at one audience says nothing about a scope at another
({{rationale-boundary}}).
...
* The hop is still active and continuable by the RAS's own authorization
  semantics.
* The authenticated actor is bound to the authorization context in which it
...
deployment designates. For a RAS acting as its own CAI that is the RAS's token
endpoint, discoverable through the RAS's metadata ({{metadata-ras}}); a
...
copied into the resulting assertion. The CAI MUST determine, from its
registration or configuration of the client, which protected resources that
client operates, and issue only for a `subject_token` valid for one of them.
...
5. Evidence that is authoritative by the RAS's own authorization semantics,
   ...
   that token's validity. A carrier another party issued, such as a
   Transaction Token, conveys the handle but is not that evidence
   ({{separate-cai}}).
...
A CAI that is not the accepting RAS MUST obtain the handle, and the evidence
...
Where the `subject_token` is a Transaction Token, that Transaction Token is
the carrier ({{handle-propagation}}), so the handle reaches the CAI from the
RAS binding rather than from the requester; the Transaction Token's own
validity is not the acceptance evidence, because the token service, not the
RAS, issued it.
...
Its `iss` is the actor's identity authority, and issuer pairing
({{validation}}, {{security-trust-model}}) is defined against that authority,
not against whichever parameter carried the credential. Where the IdP's
registration of the client supplies no other canonical actor identity, for
example a client authenticated with a client secret or a mutual-TLS
certificate and registered without a workload identity, the identity
authority is the IdP's own issuer identifier and `sub` is the `client_id`.
...
authentication alone identifies the root actor, and a root exchange carries no
...
actor identity to the assertion's `act`.
...
4. **Chain state.**
   * the handle identifies a hop the IdP issued, on an active chain, that the
     ...
     trusts for that hop's RAS is itself that attestation, which no claim
     states ({{hop-activation}});
...
7. **Authorization.** The IdP MUST issue an ID-JAG only if the governing
   ...
   under its policy. When the request omits `scope` or `resource`, the IdP
   applies the defaults its policy defines for the requested audience and,
   absent any, rejects the request with `invalid_target`
   ({{error-response}}). On an idempotent retry the IdP evaluates the
   authority granted at issuance, including the defaults applied then
...
On success, the IdP records a child hop ({{hop-activation}}) of the presented
hop and issues an ID-JAG carrying the resolved target `sub` and a fresh
handle. An idempotent retry (the freshness rule; {{idempotent-retry}}) instead
...
which the issuer-trust rule needs for the hop's RAS and tenant; the
issuer-trust rule; the chain-state rule, within which a permanently unusable
hop (ended chain, revoked hop or ancestor, withdrawn permission) precedes a
limit; the current-actor rule; the freshness rule; and the authorization
rule. A chain-state code is therefore returned only for an assertion whose
signature a configured issuer produced, and the contents of an unverified
...
a handle disabled by withdrawn permission to continue cannot re-root at all.
...
A pending idempotent retry may be repeated ({{idempotent-retry}}). Other
`invalid_request` failures, such as malformed requests or prohibited parameters,
require correcting or abandoning the request.
...
matching the fingerprint of an ISSUED reservation, the IdP MUST return the
previously issued grant unchanged when the recovery checks below succeed, and
MUST reject the request under {{error-response}} otherwise. The fingerprint
MUST cover:
...
A Resource Authorization Server advertises its support separately, by listing
the grant profile `urn:ietf:params:oauth:grant-profile:id-jag-continuation` in
its `authorization_grant_profiles_supported`
{{I-D.ietf-oauth-identity-assertion-authz-grant}}. That value signals that the
server recognizes a continuation-capable ID-JAG and binds the
`identity_continuation_handle` claim to authorization state
({{ras-processing}}); it is distinct from the base ID-JAG grant profile, which
signals only ordinary ID-JAG processing and no handle binding.
...
This section is non-normative. It describes ways an IdP, a RAS, and a CAI can
...
An IdP that offers idempotent retry also holds the fingerprint and result,
with recovery governed by {{idempotent-retry}}; an IdP that does not offer
retry needs no more than the reservation itself. Ancestry caches and audit
indexes can supplement immutable parent records, provided lineage and ancestor
revocation checks remain correct for each branch ({{onward-id-jag}}).
...
Both topologies produce the same Identity Continuation Assertion and apply the
...
that carries the handle. Neither topology changes the number of exchanges:
co-location removes configuration and trust surface, not calls, while a
separate CAI adds the carrier and the state lookup behind it.

Which carrier a deployment uses is its own choice, subject to the rule of
{{handle-propagation}}; the gateway example places the handle in the RAS's own
access token as a key into its state ({{example-gateway}}). Three carriers are
common:

* A Transaction Token {{I-D.ietf-oauth-transaction-tokens}} can carry the
  handle as request context ({{rationale-grant-type}}).
...
The assertion is not a bearer token; every exchange requires live proof of
possession of the `cnf` key, a DPoP proof {{RFC9449}} for the method this
document defines ({{validation}}, {{client-identity}}). A captured assertion
is therefore useless without the private key, and because the onward ID-JAG
and the accepting RAS's access token are bound to that same key
...
RAS runs the base profile and may issue a bearer token; the chain ends there.
...
redeemer proved the ID-JAG's `cnf` key at the RAS, which bound the handle to
its authorization state and the access token to that key ({{ras-processing}});
...
its own key (facts 2 and 3); and the CAI places that key in `cnf`. Where the
context was forwarded within the domain, carried in a Transaction Token, or
derived from durable task state for a scheduled run ({{handle-propagation}}),
fact 4 substitutes for the call-boundary check.
...
binding chain; facts 2 and 3 apply unchanged.
...
This profile adds no proof requirement at the root ({{root-establishment}});
...
to ({{assertion-token-exchange}}): the chain changes key at the call boundary
...
authentication, copied unchanged into onward ID-JAGs ({{onward-id-jag}}). A
target that gates on `acr` or `amr` should evaluate `auth_time` as well, since
on a grant-anchored chain ({{lifecycle-anchors}}) the user may be absent and
that context may be old.
...
CAI for the chain's tenant ({{client-identity}}, {{validation}}).

An accompanying CAI assertion does not relax that. CAI attestation of the hop
and authentication of the actor are independent checks, and neither
substitutes for the other ({{security-trust-model}}).
...
issuer, key, tenant, or issuer-pairing trust ({{validation}},
{{security-trust-model}}). Separating the CAI isolates keys and components but
...
The nested `act` claim is the disclosed actor lineage, a possibly minimized
...
({{onward-id-jag}}), so a rule such as "deny if a given actor ever
participated" cannot be enforced from `act` alone.
...
The onward ID-JAG's `act`, the disclosed actor lineage, also names the prior
actors to the accepting RAS outright, with no correlation needed; policy may
limit the depth disclosed to each audience ({{onward-id-jag}}).
...
comes first because it is the baseline deployment and the one a gateway
vendor implements. Authorization outcomes illustrate deployment policy, not
...
* The gateway example shows the floor, with a root client that never proves a
  key; the SaaS example shows a root RAS that sender-constrains its access
  token by its own policy ({{root-establishment}}). From the first
  continuation on, every RAS binds its token to the proven key
  ({{ras-processing}}).
...
without ever seeing Alice's credential.
...
the hops it accepts, so no separate CAI is involved. It carries the handle in
...
|---|---|---|
| IdP | `agent-app` and `tool-gateway` registered as confidential OAuth clients in `tenant-123`, with canonical actor identities (`https://agent.example/`, `agent-app`) and (`https://gateway.example/`, `tool-gateway`), the values their `act` entries carry; `https://gateway.example/` authorized as the issuer of `tool-gateway`'s credential; GatewayRAS's issuer `https://ras.gateway.example/` and its signing keys trusted for the hops it accepts; GatewayRAS and `https://gateway.example/` authorized as a CAI and actor identity authority pair for `tenant-123`, since trust in each alone is not enough; tenant policy granting agents read access to productivity tools and permitting `tool-gateway` to continue; `identity_continuation_supported` advertised; `tool-gateway`'s client identifier at WikiRAS recorded for the onward ID-JAG's `client_id` | {{client-identity}}, {{security-trust-model}}, {{root-establishment}}, {{metadata-idp}} |
...
| `tool-gateway` | a DPoP key pair for the continuation assertion's `cnf`; a client-authentication credential issued at `https://gateway.example/` | {{client-identity}} |
...
The `client_assertion` that `tool-gateway` presents to the IdP
({{example-gateway-continue}}) is an {{RFC7523}} JWT issued by the gateway
domain; the IdP maps the authenticated client to its configured actor identity.
It is addressed to the IdP alone: the earlier exchange at GatewayRAS
...
Its `iss` and `sub` here match the canonical actor identity the IdP holds for
`tool-gateway`, the values the assertion's `act` and the onward ID-JAG's
lineage carry ({{client-identity}}, {{assertion-claims}}).
...
The authorization uses the tenant's current classification of productivity
...
active and that its binding still records continuation as permitted, then
issues the assertion bound to the proven key ({{assertion-preconditions}}).
...
* Issuer trust: the assertion is signed by GatewayRAS, the RAS recorded for
  H0's hop, which that rule accepts directly.
...
from this profile: it validates the ID-JAG as the base profile requires,
ignores the handle, and issues an access token without binding H1
({{ras-processing}}). ToolGateway calls WikiAPI as Alice's wiki subject and
returns the result to AgentApp.

Further calls to WikiAPI for Alice reuse this access token while it remains
valid and covers the requested access ({{implementation}}); each further
upstream a tool call needs repeats the exchange of {{example-gateway-ica}} and
creates a sibling hop under H0 (H2, and so on).
...
* GatewayRAS binds the handle when it redeems a continuation-capable ID-JAG,
  records whether continuation is permitted, advertises the continuation grant
  profile alongside the base grants, places the handle in the access token, and
  issues assertions from its token endpoint ({{ras-processing}},
  {{assertion-issuance}}, {{metadata-ras}}).
...
an OAuth client of the IdP, holding a client-authentication credential and the
key its DPoP proofs use, and a client known to WikiRAS, which the onward
ID-JAG names as `client_id` ({{token-exchange}}).
...
credential, and WikiRAS runs the base profile unmodified. The IdP carries the
rest: it establishes the chain and evaluates each continuation
...
at `https://idp.example/`. {{example-differences}} lists what differs from
the gateway example.
...
Each domain is the identity authority for its own workloads, so the IdP has
the canonical actor identities (`https://expenses.example/`, `expense-app`),
(`https://expenses.example/`, `expense-service`), and
(`https://travel.example/`, `travel-service`) recorded for these clients, the
values their `act` entries carry ({{client-identity}}).
...
ExpenseApp redeems the ID-JAG at ExpenseRAS for an access token, AT1, and
ExpenseRAS binds H0 to the authorization state behind AT1 and records
continuation as permitted ({{ras-processing}}). The carrier is what differs:
the handle stays in this record, and AT1 does not carry it.
...
Expense CAI verifies the Transaction Token and reads H0. It checks the routing
state that assigns this transaction to `expense-service`; the domain audience
and `req_wl` alone do not establish that assignment. It rechecks with
ExpenseRAS that H0's authorization remains active and permits continuation,
then issues the assertion bound to the proven key
({{assertion-preconditions}}).
...
`expense-service` presents the assertion to the IdP with an {{RFC7523}} client
assertion and a DPoP proof of the continuation assertion's `cnf` key, requesting
an ID-JAG for TravelRAS. The IdP maps the authenticated client to its canonical
actor identity ({{client-identity}}):
...
* The CAI is a separate service in each continuing domain, which the IdP
  trusts for that domain's RAS from tenant configuration ({{issuer-trust}}); a
  co-located RAS needs no such record, though the same conjunctive trust
  applies to both ({{security-trust-model}}).
...
also shows a continuation refused for a target the grant excludes, even
though tenant policy allows it.
...
The platform domain is the identity authority for `briefing-agent`, so the
IdP has the canonical actor identity (`https://platform.example/`,
`briefing-agent`) recorded for that client, the value its `act` entry carries
({{client-identity}}).
...
{{lifecycle}}). In this deployment, the grant permits only the Platform and
Calendar access needed for the task, and current policy permits that access.
The root ID-JAG targets PlatformRAS.
...
`briefing-agent` is both the root actor and the current actor here, so the
onward `act` merges them into one entry ({{onward-id-jag}}). CalendarRAS is
terminal and issues the access token without binding the child hop. Each run's
child is a sibling, not a descendant, of the previous run's child.
...
access does not by itself expand that grant ({{validation}}).
...
`invalid_scope` if that authorization or current policy excludes it. These
...
continuable for other authorized targets.
...
5. **Stateless hop commitments.** Could a handle be a self-verifying
   commitment carrying the root, parent, RAS, and actor, so that the IdP
   keeps only root, revocation, and reservation state? What would the working
   group need to see? How the IdP recovers a hop's ancestry when the parent
   record no longer exists, given that lineage is derived from each hop's
   ancestry and an ancestor's revocation has to be detectable
   ({{onward-id-jag}}, {{lifecycle}}); how it does so within the handle's
   256-character bound ({{chain-id}}) without embedding commitments
   recursively; and what a revocation list costs in subtree revocation and
   unlinkability ({{privacy}}).
...
Across Domains and the Identity Assertion JWT Authorization Grant
specifications, on whose work this profile builds.
...
* Replaced the root-chain envelope and authorization basis with the chain's
  ...
  authority including defaults the IdP applies; policy can narrow but not exceed
  what the governing authorization permits.
* Removed `actor_token` processing from both exchanges. Client authentication
  ...
  derives from its registration of the client.
* Made `act` the disclosed actor lineage, distinct from the IdP's authoritative
  hop lineage; the bound on it is the actor-lineage depth bound.
* Replaced the three hop states with two facts, issuance at the IdP and
  acceptance in RAS authorization state. A hop is continuable while a CAI
...
* Removed the root exchange's proof-of-possession requirement; a root ID-JAG
  need not carry `cnf`. Onward ID-JAGs are redeemed with the DPoP-bound JWT
  grant, which a continuation-aware RAS advertises.
* Assertion and handle rules: `nbf` permitted; the 300-second lifetime bound and
  the 256-character handle bound recommended rather than required, with the IdP
  rejecting a lifetime above its maximum; `jti` needs collision resistance and a
  handle 128 bits of unpredictability; the accepting RAS's access token or
  another intra-domain carrier may carry the handle, which is also an
  introspection response member.
* Kept assertions single-use, made idempotent retry optional with a request
  fingerprint, prohibited a refresh token in both responses, and added recovery
  guidance after a lost response and for `invalid_request` in general.
* Distinguished the errors: `unauthorized_client` for an actor not permitted,
  `invalid_continuation` only for a permanently unusable handle, `invalid_grant`
  for limits, and `invalid_target` for an unpermitted or unresolvable target.
* Lifecycle gathered into one section: grant anchors optional; a subject token
  that resolves no anchor yields an ID-JAG without a handle; a chain never
  outlives its anchor and an ended chain stays ended; RAS-local withdrawal does
  not revoke descendants accepted at other RASes; limits configured for a
  governing authorization apply across every chain rooted in it.
* Removed the RAS `identity_continuation_issuers` nomination, its
  registration, and the Metadata Disclosure security consideration that -01
  added with it. The IdP configures CAI trust per tenant, scoped by issuer,
  keys, tenant, and the RAS attested for, with `jwks_uri` resolution under a
  bounded key cache.
* Reorganized Security Considerations around the current model (authorization
  enforcement, topology and trust, trust in actor identity authorities) and
...
* Protocol fixes: the replay reservation is atomic, shared across IdP instances
  ...
  and every chain a finite hop-count limit, both recorded as an open item; the
  IdP verifies serialization and the CAI verifies subject-token type; error
  codes are mandatory, with precedence defined around rule dependencies;
  RESERVED and FAILED reservations and a single clock-skew value are specified;
  limit counters are defined per hop, per chain, or per governing authorization.

* Review follow-ups: a RAS binds the tenant where its deployment conveys one,
  whose derivation is deployment-defined; a separate CAI has to take acceptance
  and continuation-permitted state from a source authoritative for the RAS,
  since a Transaction Token's own validity is the token service's word rather
  than the RAS's; the CAI determines from registration or configuration which
  protected resources a client operates; the client now has to confirm that the
  discovered issuer matches `identity_continuation_authorization_server`, and
  the IdP to record each hop's RAS audience and the issuers it trusts for it.
  Stated that a trusted CAI's assertion is itself the acceptance attestation,
  what the IdP does when `scope` or `resource` is omitted, and that a target
  gating on `acr` or `amr` should weigh `auth_time` too. Gathered the binding
  chain into Security Considerations and handle carriage into Handle Carriers,
  named the design choice, the continuation-aware RAS's obligations, and the
  three exchanges in scope in the Introduction, reworded the
  stateless-commitment open item as a question, and made the DPoP-bound JWT
  grant reference normative.
