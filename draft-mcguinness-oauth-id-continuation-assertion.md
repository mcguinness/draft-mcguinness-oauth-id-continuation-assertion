...
of these exchanges assumes the subject's credential (an ID Token, refresh
token, or SAML assertion) is present when the grant is minted.
...
Each trust domain from which the chain continues has two roles: a RAS that
...
Between them, a trusted intra-domain carrier surfaces the accepted hop's handle
to workloads inside the domain ({{transaction-token-context}}); the carrier is a
